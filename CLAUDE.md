...
## Message Formatting with aiogram

**Important**: Telegram MarkdownV2 is **NOT** standard Markdown. It has complex escaping rules, but this project uses **aiogram formatting utilities** that handle all escaping automatically - **no manual escaping needed**.

### The Key Rule

**Always use aiogram formatting components** - they manage all MarkdownV2 escaping automatically:

```python
from aiogram.utils.formatting import (
    Text, Bold, Italic, Underline, Strikethrough, Code, Pre,
    as_list, as_marked_section, as_key_value
)

# No manual escaping needed - aiogram handles it automatically
content = Text(
    Bold('Price:'), ' ',
    Strikethrough('$99.99'), ' → ',
    Bold('$49.99'), '!',
    '\n',
    Italic('Limited time offer!')
)
...
| `Pre('code', language='python')` | Code block | ```code``` |
| `Link('text', url='...')` | Hyperlink | [text](url) |
| `Text('a', Bold('b'), 'c')` | Combine | a**b**c |

### Structured Formatting

```python
# Build organized lists and sections
content = as_list(
    as_marked_section(
        Bold('Success:'),
        'Test 1',
...
**User input, agent responses, and markdown notes** - wrap everything in formatting components:

```python
# Safe handling of user input with special characters
user_message = 'Price: $99.99! Buy now -> click here'
content = Text('You said: ', user_message)  # Automatically escaped

# Agent responses with mixed formatting
response = Text(
    Bold('Agent:'), ' ',
    agent_reply,  # No manual escaping needed
    '\n\n',
    Italic('Reasoning:'), ' ',
    reasoning_content
)

# Markdown notes loaded from files
with open('notes/help.md') as f:
    help_text = f.read()
content = Text(help_text)  # aiogram handles all special chars
```

### Critical Points

1. **Never build raw MarkdownV2 strings** - always use aiogram formatting components
2. **No manual escaping required** - aiogram automatically handles all special characters
3. **User input safety** - wrap any user input or external text in `Text()` for safe rendering
4. **Nested formatting** - compose freely by nesting components: `Bold(Italic('text'))`

### Reference

- aiogram formatting docs: https://docs.aiogram.dev/en/v3.22.0/utils/formatting.html
...
• Request agent access via /request\_resource
